...
def main():
    ...
    raw_results   = {}   # 정책별 100회 LCC 리스트 (통계 검정용)
    results_table = {}   # 출력용 요약 딕셔너리
    ...
    for label, mode in ablation_modes.items():
        m_c, s_c, m_f = simulator.run_monte_carlo(
            policy_type="제안", ablation_mode=mode, iter_num=100
        )
        results_table[label] = {
            "평균 LCC": m_c,
            "표준편차": s_c,
    ...
    # ── 통계 검정 & p-value 매핑 (버그 수정) ──────────────────────────────────
    # perform_statistical_tests는 {정책명: p_value} 딕셔너리를 반환
    # 기준 정책(법정)은 "—", 나머지 3개에 각자 p-value 매핑
    _, p_post = perform_statistical_tests(raw_results)

    results_table["법정"]["p-value"]     = "—"
    results_table["임계값"]["p-value"]   = p_post.get("임계값",   None)
    results_table["Frangopol"]["p-value"] = p_post.get("Frangopol", None)
    results_table["제안"]["p-value"]     = p_post.get("제안",     None)   # ← 버그 수정

    # ── [3] 표 출력 ───────────────────────────────────────────────────────────
    UNIT = 1e8   # 억 원 단위 변환

    def get_width(text):
        """한글(2) / 영문·숫자·기호(1) 기반 실제 출력 폭 계산"""
        width = 0
        for char in text:
